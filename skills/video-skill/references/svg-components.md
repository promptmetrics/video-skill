...
Create reusable, animated SVG components for educational video visuals.
...
const FORCE_CONFIG = {
  lift: { color: '#4facfe', label: 'Lift', direction: 'up' },
  gravity: { color: '#fa709a', label: 'Gravity', direction: 'down' },
  thrust: { color: '#38ef7d', label: 'Thrust', direction: 'right' },
  drag: { color: '#eb3349', label: 'Drag', direction: 'left' },
} as const;
...
<AnimatedBarChart
  data={[
    { label: 'China', value: 1400, color: COLORS.accent.rose },
    { label: 'India', value: 1380, color: COLORS.accent.teal },
    { label: 'USA', value: 331, color: COLORS.accent.yellow },
    { label: 'Japan', value: 126, color: COLORS.semantic?.neutral ?? '#74b9ff' },
  ]}
  startFrame={30}
/>
...
When building educational video visuals, **every scene needs at least one visual illustration** — not just text labels in colored boxes. This section shows how to create Kurzgesagt-style domain illustrations and upgrade common "PPT-like" patterns.
...
//   <span>Talent War</span>           // <- This is the PPT anti-pattern
// </div>
```
